...
| MobilityData/gtfs-realtime-bindings (Go) | pseudo-version, see `go.mod` | Generated protobuf structs for GTFS-realtime | Hand-generating from the `.proto` | The module is `github.com/MobilityData/gtfs-realtime-bindings/golang/gtfs`. **Note:** at the time of writing this module has no semver tags, so `go.mod` will carry a `v0.0.0-<date>-<commit>` pseudo-version. That is expected, not a mistake. See §9.1 for why the TfNSW feeds may still need a locally generated binding. |
...
- Bounded: entries expire after `CACHE_TTL`. A background sweep runs every minute.
...
| `INGEST_QUEUE_SIZE` | int | `8192` | no | Too small and the matcher blocks during a write stall, which backs pressure into the pollers. Too large and a crash loses more rows and the process holds more memory. |
...
| Order | Key | Produces |
|---|---|---|
| 1 | `trip_id` + `stop_sequence` | Full match. Scheduled time known. |
...
|---|---|---|
| Updates ingested per minute | `rate(headway_updates_decoded_total[5m]) * 60`, peak hour | baseline TBD (Stage 1) |
| Rows written per minute | `rate(headway_rows_written_total[5m]) * 60` | baseline TBD |
| Change-filter suppression ratio | `headway_filtered_total / (headway_filtered_total + headway_admitted_total)` | ≥ 0.90 |
...
| Storage per day, raw | `pg_total_relation_size('observations_YYYY_MM_DD')` | baseline TBD (Stage 3) |
