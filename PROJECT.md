...
| `HTTP_CLIENT_IP_HEADER` | header name | empty | no | The header the per-IP rate limit and the access log take the client's address from, when the only thing that can reach the port is a proxy that sets it. Empty uses the connection's address. The VM sets `CF-Connecting-IP`: it publishes no public port and is reached only through the Cloudflare Tunnel. Setting it on a public port would let any client choose its own bucket. |
