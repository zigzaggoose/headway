...
│   ├── 0005_observations_key.sql     Key on stop_id; stop_sequence nullable (§16 q11).
│   └── 0006_observations_scheduled_at.sql  scheduled_at, the rollup's bucket time (§16 q12).
│
├── config/
...
│
├── web/                              Next.js static export on Cloudflare Pages (§15): app/page.tsx, chart.tsx, api.ts.
│   └── (Next.js app)
│
└── .github/
    └── workflows/
...
CREATE TABLE observations (
    ...
    -- ADDED by migrations/0006: scheduled_at timestamptz, the visit's
    -- timetable time, NULL when unmatched. The rollup buckets by it (§16 q12).
...
Base URL `https://api.transitlateagain.dev` (the site, `web/`, is at `https://transitlateagain.dev`), base path `/v1`. When `HTTP_CORS_ORIGIN` is set, every public response carries `Access-Control-Allow-Origin` with exactly that origin and `Access-Control-Expose-Headers: Retry-After, X-Request-Id`, so the site can read errors and 429s too; every endpoint is a simple GET, so browsers never preflight and there is no `OPTIONS` handling. All responses `application/json; charset=utf-8`. All timestamps RFC 3339 with offset. Example ids below are **illustrative**: real `route_id` and `stop_id` values come from the TfNSW bundle and must never be hard-coded in application code, tests excepted.

#### `GET /v1/lines`

List routes in the active schedule that have at least one trip. TfNSW's bundle carries routes with none (34 of 152 train routes on 2026-09-24); since 2026-09-24 they are not loaded at all, so they are absent here and `/now` answers `404 line_not_found` for them (§15).
...
**Since 2026-09-26:** a trip the feed calls `SCHEDULED` or `REPLACEMENT` that is not in the active timetable is not listed, and not counted in `summary` or at its stop: it is a finished trip from a replaced timetable (§9.1 case 18). Unmatched trips that remain are `ADDED`, cancelled, or carry a relationship value nobody has seen yet.

#### `GET /v1/stops/{stop_id}/now`

Upcoming departures at a stop with live delay.

Every call the feed still lists for a trip is one not yet served, so there is no lower bound on time: a late train stays listed until the feed drops it. A departure is included when its predicted time (or scheduled time, with no delay) is within `window_min` of now, sorted by that time. Unmatched calls have no scheduled time and are left out. `status` is as for `/v1/lines/{route_id}/now`, including `cancelled`; `predicted` is `null` without a delay. `404 stop_not_found` means the stop is in no loaded timetable; `503 not_ready` means none is loaded. `as_of` is `null` before the first poll.
...
| `HTTP_CLIENT_IP_HEADER` | header name | empty | no | The header the per-IP rate limit and the access log take the client's address from, when the only thing that can reach the port is a proxy that sets it. Empty uses the connection's address. The VM sets `CF-Connecting-IP`: its firewall admits only Cloudflare's ranges to the API port. Setting it on a port anyone can reach would let any client choose its own bucket. |
| `HTTP_CORS_ORIGIN` | origin | empty | no | The one site allowed to read the API from a browser, as `scheme://host` with no path or trailing slash (config refuses one, because a browser would never match it). Empty sends no CORS header. The VM sets `https://transitlateagain.dev`; `web/` against a local API uses `http://localhost:3000`. |
| `HTTP_TLS_CERT_FILE`, `HTTP_TLS_KEY_FILE` | path | empty | no | Serve HTTPS on `HTTP_ADDR` with this pair; both or neither, loaded at startup so a bad file refuses to start. Empty serves plain HTTP. The VM uses Cloudflare's origin certificate, whose key was generated on the VM. |
...
| Sydney Trains schedule | `https://api.transport.nsw.gov.au/v1/gtfs/schedule/sydneytrains` | `200`, `application/zip`, 10,715,169 bytes (11,292,495 on 2026-09-23), `content-disposition: attachment; filename=sydneytrains_GTFS_PROD_<YYYYMMDDHHMMSS>.zip`. |
...
- The **schedule** response carries `Last-Modified` (no `ETag`), but an `If-Modified-Since` it has not moved past is answered with **502, not 304** (measured 2026-09-24; an older date gets 200), so conditional requests cannot save a download and none is sent (§15). The bundle is regenerated daily with new file timestamps inside the zip and a calendar starting that day, so in practice every day is a new content hash and a new version.
...
| **The timetable matches the feed** | fresh response vs same-day bundle, 2026-09-23: `(trip_id, stop_id)` found in `stop_times` for **2,886 of 2,912** stop updates (99.1 %); `route_id` 2,912 of 2,912; `stop_id` 2,912 of 2,912; `trip_id` for 301 of 309 trips. Of the 19 updates whose trip is absent, 10 are `ADDED`. | The operational route ids (`NSN_1a`, `APS_1a`…) are the bundle's own `route_id`s, so no mapping is needed. The feed sent `start_date` on **0** updates, so the service date always comes from `CandidateServiceDates` plus the schedule. |
| **The other modes differ** (2026-09-23) | Metro, ferries and both light rail lines send `start_date` and `stop_sequence` on every update, unlike trains, so match orders 1 and the date from `start_date` are live paths. CBD & South East light rail cancels trip variants whose ids (`…-R:X`) are not in its bundle and adds replacements as `…-R-R:X`: 18 cancelled trips of that shape made its match rate 89.7 % while every scheduled trip matched. | Not a matcher fault; the match-rate definition is unchanged. Seven stop ids are shared across feeds — parent stations such as Central (`200060`) — which merge in stop history, correctly. No route id is shared. |
...
| **When bundles are published** (`schedule_versions.last_modified`, 2026-09-23 to 09-26) | Trains 01:01 and ferries 01:02–01:03 every night; metro 03:03–03:18 and light rail 02:47–03:01, plus one metro/light-rail publish at 12:55 on 09-24. The feed keeps a replaced timetable's trips after the bundle drops them (§15, 2026-09-26). | The 03:30 load follows every nightly publish, the metro one by as little as 12 minutes. Recorded only when content changed and only as often as the loader checked, so a midday republish would not show here. |
...
- *Query Postgres per update instead of an in-memory cache.* Rejected on measurement grounds before measuring: at four polls a minute across several feeds this is tens of thousands of point lookups a minute on a shared free-tier disk. The whole active schedule for the trains feed is **1,244,526 stop-times across 68,738 trips** (measured 2026-09-23 — the earlier "few hundred thousand" was a guess). That still fits in memory, but not casually on a 1 GB VM: the matcher should hold only trips whose service runs on a candidate service date, not the whole bundle.
...
18. The producer keeps a trip after it has finished, or keeps its served stops ahead of the next one → a stop whose scheduled time plus observed delay is more than 5 minutes behind the feed timestamp is served (10 minutes on first deploy; see §15); the latest-state cache drops it, so a trip's next stop is its first unserved one and a trip with none left is not active. Unmatched trips have no scheduled time to judge by; an unmatched trip the feed calls `SCHEDULED` (0) or `REPLACEMENT` (5) is a ghost and is dropped too, while `ADDED`, cancelled and unknown relationships are kept (since 2026-09-26, §15). Found live on 2026-09-24: 160 of 748 trips `/now` called active had a next stop over 30 minutes past, up to 13.7 h; in one ferries message 69 of 134 trips had every stop more than 10 minutes past and 19 more led with served stops (trains: 25 and 8 of 354). Storage is unaffected: the rollup already keeps one row per trip and stop. `TestUpdate_ServedStops_AreNotNextStopsOrCalls` and `TestUpdate_UnmatchedTrips_OnlyGhostsAreDropped` in `internal/cache`.
...
- [x] Every §9.1 edge case has a named subtest and passes. Cases 1–5 and 7–15, 17 in `internal/match`; 6 in `internal/gtfsrt`; 16 in `internal/gtfsstatic`. Case 12 is now counted (`Counts.UnknownStop`); case 14 was not implemented as specified — the database kept the *first* duplicate via `ON CONFLICT DO NOTHING` — and now resolves last-wins in the matcher.
...
- [x] Add bus, metro, light rail and ferry feeds to `config/feeds.json`, each verified with `curl` first. Metro (v2 — the v1 bundle is from 2024), Sydney Ferries, CBD & South East and Parramatta light rail enabled; buses listed but disabled by the user's decision (§15). No Inner West light rail endpoint exists (404); its `IWL` routes arrive in the trains feed. Live, 2026-09-23: match rates 100 % (metro, ferries, Parramatta), 99.6 % (trains), 89.7 % (CBD, below — every miss is a cancellation of a trip variant absent from the bundle, §9.1); 196 MB RSS for all five.
...
- [x] One dashboard: ingest rate, freshness p95, match rate, queue length, partition count, API p95. 2026-09-24: `deploy/grafana-dashboard.json`, 16 panels, imported into Grafana Cloud. A V1 dashboard resource (`dashboard.grafana.app/v1`): Grafana 12.2+ calls bare classic JSON the old format. Every query was evaluated against live data before import.
- [x] One alert wired end to end — `TransitLateAgainIngestStopped` — with a `docs/runbook.md` entry. 2026-09-24: a Grafana-managed rule (settings in the runbook), email contact point, a test notification received. The expression was shown to fire on an absent metric and stay quiet on live data; it has not been fired by stopping ingestion, by the user's choice, because that window is data TfNSW never archives.
- [x] `web/`: Next.js, one page, a line selector and a stop history chart hitting the live API. 2026-09-24: https://transitlateagain.dev on Cloudflare Pages, reading `api.transitlateagain.dev` (CORS for that one origin). The stop chart opens from a trip's next stop, filtered to the line (§15).
...
| Freshness lag p95 | `histogram_quantile(0.95, ...)` | ≤ 30 s. **First measurement 2026-09-24 ~14:35, VM, five feeds, a 5-minute `rate` window over ~6 minutes of scrapes: 17.7 s** overall; by feed 13.1 (metro) to 28.0 (ferries). A short window at one time of day — a baseline, not a claim. |
...
| API p99 latency, `/now` | same run | ≤ 250 ms. **Measured 2026-09-24: 2.2 ms.** |
...
| 2026-09-24 | The per-IP rate limiter (§7.2's 429, `HTTP_RATE_LIMIT_RPS`) is built: a token bucket per `RemoteAddr` holding one second of requests, idle buckets swept on use, health probes exempt, inside the request-id middleware. Asked and approved (a new lock). | Promised by the 2026-09-23 Stage 1 row and not delivered until the load-test preparation noticed. `X-Forwarded-For` is not trusted: nothing sits in front of the service, so the header is whatever the client sends. | Dropping 429 from §7.2; a sweep goroutine; keying on `X-Forwarded-For`. |
...
| 2026-09-24 | Schedule downloads are unconditional: no `If-Modified-Since`. Supersedes the schedule half of the 2026-09-21 row ("`If-Modified-Since` on the schedule download only"). | TfNSW answers an `If-Modified-Since` the bundle has not moved past with 502, not 304 (three requests on `lightrail/parramatta`: plain 200; its own `Last-Modified` 502 twice; a 2024 date 200). So every refresh after the first failed, logged ERROR, and retried all five feeds every 15 minutes, which is the "trains schedule endpoint returns 502 often" of 2026-09-23. After a 502 the bundle has to be downloaded anyway to tell unchanged from an outage, so the condition never saves a download; the content hash already makes an unchanged bundle a no-op. Found through `feed_requests_total{outcome="error"}` on the first live scrape. | Treating a 502 to a conditional request as 304 (a real outage on a changed bundle would then keep yesterday's timetable for a day); falling back to an unconditional download after the 502 (two requests where one does). |
| 2026-09-24 | `transitlateagain.dev` reaches the VM through a **Cloudflare Tunnel** (`cloudflared` in Compose, profile `vm`), and the VM publishes no public port: 8080 is off the base Compose file (the local override keeps it) and only SSH is open. The per-IP connection limits (`deploy/firewall.sh`, `transitlateagain-firewall.service`) go with it. The rate limiter keys on `CF-Connecting-IP` via `HTTP_CLIENT_IP_HEADER`. | The tunnel is outbound only, so the origin cannot be reached around Cloudflare at all, which is what makes trusting the header safe; TLS runs end to end with no origin certificate; and no list of Cloudflare's address ranges has to be kept in a firewall. `.dev` is HSTS-preloaded, so the domain needed TLS anyway. About 30 MB of memory. | Flexible SSL to a published port (Cloudflare to origin in plain HTTP, and the origin still reachable directly unless every Cloudflare range is allowlisted); Full (strict) with an origin certificate (Caddy or Go TLS, a private key to manage, and the same allowlist); Caddy with Let's Encrypt and no Cloudflare (no flood absorption). |
| 2026-09-24 | Grafana Alloy v1.19.2 on the VM (profile `vm`) scrapes `/metrics` every 60 s over the Compose network and remote-writes to Grafana Cloud; `GOMEMLIMIT=96MiB`. | `/metrics` stays on the private admin port. 60 s is one sample per series per minute, the free tier's allowance; 320 series against its 10,000. | Grafana Cloud scraping a public `/metrics` (it would need its own auth, a §2 non-goal, and a public port); Prometheus in agent mode (its config cannot read the token from the environment, so a secret file readable by its user); the service pushing itself (a remote-write client in the binary). |
| 2026-09-24 | Supersedes the Cloudflare Tunnel row: Cloudflare proxies `transitlateagain.dev` straight to the VM in **Full (strict)** mode. The service serves Cloudflare's origin certificate (15 years; key generated on the VM from a CSR, so it never left it) on 443 → 8080, and `deploy/firewall.sh` admits only Cloudflare's IPv4 ranges to that port, replacing the per-IP connection caps, which would have throttled Cloudflare's own addresses. The VM's differences live in `deploy/docker-compose.vm.yml` (443, the certificate, `CF-Connecting-IP`, Alloy) instead of a Compose profile. | Cloudflare's Zero Trust signup, which the tunnel needs, asked the user for a card. TLS end to end and an origin only Cloudflare can reach are kept; the costs are a list of 15 ranges to refresh if Cloudflare changes them and a certificate to replace in 2041. | The tunnel (a card on file); Flexible SSL (plain HTTP from Cloudflare); Authenticated Origin Pulls instead of the range list (client-certificate code in the service for what one iptables chain does). |
| 2026-09-24 | `web/` is a Next.js 16 static export hosted on Cloudflare Pages at `transitlateagain.dev`, built from GitHub on every push; the API moves to `api.transitlateagain.dev` and sends a single-origin CORS header (`HTTP_CORS_ORIGIN`). One page: a line picker, the line's live trips, and an on-time chart for the line or, after picking one of its next stops, that stop filtered to the line. No SWR, Tailwind, ESLint or chart library: `fetch` in effects, plain CSS, `next build`'s type check, and hand-drawn SVG bars. | Asked; the user chose Pages. No card, no load on the 1 GB VM, and Cloudflare already fronts the domain. The origin certificate already covers `api.`. Stop history needs a stop, and the only list of a line's stops the API has is its trips' next stops, so the stop is picked there. | Serving the export from the Go service (a Node build the VM cannot afford, and every web change a server deploy); Vercel (a second account for the same result); a stops-of-a-line endpoint (a §7 addition for a picker the next-stop list already provides). |
| 2026-09-24 | `web/` is served by **Cloudflare Workers static assets** (`web/wrangler.jsonc`, no Worker code, `out/` as the assets, `404.html` for misses), built and deployed by Workers Builds from GitHub, instead of Cloudflare Pages. Supersedes the hosting half of the previous row; everything else in it stands. | Cloudflare's dashboard now routes "connect a repository" to Workers. The same static files, the same free plan and account; one config file instead of a hunt for the Pages flow. Checked with `wrangler deploy --dry-run` (28 files). | Finding the Pages flow (no difference to the result). |
...
| 2026-09-25 | The site puts the history pie on the left and the live list on the right (stacked below 900 px). The live summary counts are toggle filters, a search box narrows trains to those whose start, end or next stop contains every word typed, the list scrolls in a box capped at 65 % of the window height, and a **Full screen** button (Esc leaves) lays the live section over the whole window with the list uncapped. | Asked for by the user: with 30 light-rail trips the list filled several screens and pushed the chart out of sight; side by side, the summary stays in view while a train is opened. | The browser Fullscreen API: iPhone Safari allows it only for video, while a fixed-position overlay works everywhere with the same state. Separate filter chips: the summary counts already name each status and how many trains it leaves. Searching by time or trip id: riders look for stations. |
| 2026-09-26 | The latest-state cache drops **ghost trips**: unmatched observations whose `trip_rel` is `SCHEDULED` (0) or `REPLACEMENT` (5). They are still stored. Refines §9.1 case 18, which kept every unmatched trip. | At 20:15 on Saturday 2026-09-26, 82 of 299 train trips on `/now` read "To an unknown stop". Every one was `817.160.64.*`, a Friday-only service dated 2026-09-25 with stop times 24:47–26:33, finished 18 hours earlier; TfNSW's feed still sent them with fresh timestamps. The bundle published at 01:01 that morning had rightly dropped timetable 817, so they could not match, and without a scheduled time case 18 kept them indefinitely. A trip that claims a timetable slot the timetable lacks is by that claim not a real service today. | Refreshing the schedule hourly (tried first, reverted before commit: the trips were not missing from a stale bundle, and loading the 01:01 bundle before the previous day's post-midnight trains finish would unmatch trains still running); matching against the previous version as well (~115 MB more on a 1 GB VM); relabelling them on the site (keeps 82 false trains in the list and the counts). Cost accepted: a real train TfNSW runs as `SCHEDULED` but leaves out of its bundle disappears from `/now`; on 2026-09-23 the bundle held 301 of 309 live trip ids, and 10 of the 19 absent updates were `ADDED`. |
...
| 12 | **ANSWERED 2026-09-23 — see the Decision Log.** **Which hour does a stop visit belong to in the rollup?** Rows are bucketed by the `feed_ts` of the visit's last observation (§6.3). For a normal trip that is when the train passed. But TfNSW keeps cancelled (`trip_rel` 3) and replacement (5) trips in the feed long after their time: at 23:00 on 2026-09-23, 122 of 143 cancellation rows and 95 of 303 replacement rows were more than 6 h past their scheduled time, so this morning's cancellations are counted in the 23:00 bucket. `SCHEDULED` trips were fine (1,108 of 1,108 within 6 h, 96 % within 1 h). | **Decided by the user: the proposal below.** Was:  bucket by `feed_ts`. **Adopted:** store `scheduled_at` on matched observations (new migration) and bucket by `coalesce(scheduled_at, feed_ts)`, recomputing the last few hours on each tick (`ON CONFLICT DO UPDATE` already makes that safe). This changes what the history endpoints mean — "visits scheduled in this hour" rather than "visits last reported in this hour" — so it needs the user's call. |
