...
- The **schedule** response carries `Last-Modified` (no `ETag`), so `If-Modified-Since` is worth sending on the daily ~11 MB download. The bundle is regenerated daily with new file timestamps inside the zip and a calendar starting that day, so in practice every day is a new content hash and a new version.
