...
Not yet built: `internal/servicetime`, `internal/match`, `internal/cache`,
`internal/api`, `internal/rollup`, `internal/obs`, `deploy/`, `web/`. §5 is the
destination layout. Create a package when its first real file is written — no
placeholder files.
